...
fn greedy_is_deterministic() {
	println!("TEST: greedy_is_deterministic");
	let shared = Shared::get();
	let forge = &shared.forge;
	let searcher = EnginesManager::new(forge.clone());
	...
	let mut engine = Greedy::new(Arc::clone(&forge), searcher.get_constrains());
	let res = engine.run();
	let mut engine2 = Greedy::new(Arc::clone(&forge), searcher.get_constrains());
	let res2 = engine2.run();
	assert!(*res.first().unwrap() == *res2.first().unwrap());
}